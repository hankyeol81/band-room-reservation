...
function Admin(){
 ...
 async function create(e){e.preventDefault();setMsg('');const f=new FormData(e.currentTarget);try{await call({action:'create_fixed_schedule',team_name:f.get('team').trim(),day_of_week:Number(f.get('day')),start_time:f.get('start'),end_time:f.get('end'),start_date:f.get('from'),end_date:f.get('to')});e.currentTarget.reset();setMsg('고정 시간표를 추가했습니다.');load()}catch(e){setMsg(e.message)}}
