...
const fixedOccupies=(f,d,t)=>{
  ...
  // Same-day fixed schedule.
  if(e>s){
    return d.getDay()===Number(f.day_of_week)&&ymd(d)>=f.start_date&&ymd(d)<=f.end_date&&m>=s&&m<e;
  }
  // Overnight schedule: e.g. Tue 23:00-Wed 01:00.
  // Evening portion belongs to the configured weekday; after-midnight portion belongs to the next day.
  const prev=addDays(d,-1);
  const onStartDay=d.getDay()===Number(f.day_of_week)&&ymd(d)>=f.start_date&&ymd(d)<=f.end_date&&m>=s;
  const afterMidnight=prev.getDay()===Number(f.day_of_week)&&ymd(prev)>=f.start_date&&ymd(prev)<=f.end_date&&m<e;
  return onStartDay||afterMidnight;
};

function ReservationForm({modal,err,onReserve}){
 const[start,setStart]=useState(modal.start);
 const startIndex=slots.indexOf(start);
 const endOptions=[1,2,3].map(n=>slots[(startIndex+n)%48]);
 const[end,setEnd]=useState(endOptions[1]);
 useEffect(()=>{setEnd(slots[(slots.indexOf(start)+2)%48])},[start]);
 return <><h2>합주실 예약</h2><p className="sub">{modal.day.getMonth()+1}월 {modal.day.getDate()}일</p><form onSubmit={onReserve}><label>팀명<input name="team" required maxLength="30" placeholder="예: A팀"/></label><div className="row"><label>시작<select name="start" value={start} onChange={e=>setStart(e.target.value)}>{slots.map(x=><option key={x} value={x}>{x}</option>)}</select></label><label>종료<select name="end" value={end} onChange={e=>setEnd(e.target.value)}>{endOptions.map(x=><option key={x} value={x}>{x}</option>)}</select></label></div><label>취소 암호<input name="pw" required type="password" minLength="4" placeholder="4자리 이상"/></label>{err&&<p className="error">{err}</p>}<button className="primary">예약하기</button></form></>;
}
...
function Admin(){
 ...
 async function create(e){e.preventDefault();setMsg('');const form=e.currentTarget;const f=new FormData(form);const from=f.get('from');const to=f.get('to');try{await call({action:'create_fixed_schedule',team_name:f.get('team').trim(),day_of_week:Number(f.get('day')),start_time:f.get('start'),end_time:f.get('end'),start_date:from,end_date:to});setMsg('고정 시간표를 추가했습니다.');await load();if(form&&form.isConnected){const team=form.elements.namedItem('team');if(team)team.value='';}}catch(err){setMsg(err.message)}}
